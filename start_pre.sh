...
#   ./start_pre.sh --init=true     -> QUARTZ_INIT_SCHEMA=always (BD nueva, bootstrap)
#   ./start_pre.sh --init=false    -> QUARTZ_INIT_SCHEMA=never  (normal, el 99% de las veces)
#
...
usage() {
    ...
    echo -e "  ${Yellow}./start_pre.sh --init=true${Color_Off}    # primera vez / BD nueva (bootstrap del esquema de Quartz)"
    echo -e "  ${Yellow}./start_pre.sh --init=false${Color_Off}   # arranque normal (el 99% de las veces)"
    echo ""
    echo -e "${BWhite}Ejemplo:${Color_Off}"
    echo -e "  ${Yellow}./scripts/start_pre.sh --init=false${Color_Off}"
}
...
run_cmd docker compose -f docker-compose.yml --env-file .env.pre up -d --remove-orphans
