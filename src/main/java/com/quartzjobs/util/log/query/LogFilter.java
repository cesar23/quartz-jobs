 ...
 *
 * Cada campo es opcional (Set vacío = "no filtrar por esto"). Dentro de un
 * mismo campo, varios valores separados por coma se combinan con OR
 * (ej: level=ERROR,WARN -> ERROR u WARN). Entre campos distintos se
 * combina con AND (ej: jobGroup=SYNC & level=ERROR,WARN -> jobGroup=SYNC
 * Y (level=ERROR O level=WARN)).
 */
public final class LogFilter {
    ...
    private LogFilter(LogFilterRequest req) {
        ...
        this.messageContains = (req.message() == null || req.message().isBlank())
            ? null
    ...
    /** Divide "A,B,C" en un Set. Si uppercase=true, normaliza a mayúsculas (útil para "level"). Set vacío = sin valores. */
    ...
    /**
     * Parsea un timestamp ISO-8601 con offset, igual al formato que escribe
     * LogstashEncoder en "@timestamp" (ej: "2026-08-10T16:11:02.949-05:00").
     * Si el valor no es parseable, se ignora ese filtro (no rompe la
     * consulta) y se deja constancia en el log.
     */
    private static OffsetDateTime parseTimestamp(String raw, String paramName) {
        ...
        try {
            ...
            log.warn("Parámetro '{}' no es un timestamp ISO-8601 válido: '{}' — se ignora este filtro", paramName, raw);
            return null;
        }
    }
    ...
    /** true si "allowed" tiene valores y "value" no está entre ellos. Set vacío = no filtra, nunca excluye. */
